...
import { authSlice, userLogin } from '../slices/auth/authSlice'
import { kgsSlice } from '../slices/kgs/kgsSlice'

export const messageStreamer = createListenerMiddleware()

messageStreamer.startListening({
  actionCreator: userLogin.fulfilled,
  effect: async (action, { delay, dispatch, subscribe, unsubscribe }) => {
    unsubscribe()
      ...
      for (const message of messages) {
        switch (message.type) {
          ...
          case KGS.KGSMessageType.USER_REMOVED:
            dispatch(
              kgsSlice.actions.userRemoved(
                message as KGS.KGSMessage_UserRemoved,
              ),
            )
            break
